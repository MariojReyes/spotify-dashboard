---
title: "This Week's Billboard Hot 100 Chart"
toc: false
format:
  html:
    page-layout: full
---

```{r}
#| include: false

library(httr2)
library(jsonlite)
library(tidyverse)
library(gt)
library(purrr)

hot100_df <- fromJSON("data/hot100_ojs.json")
```

```{=html}
<style>
.dashboard-shell {
  max-width: 1200px;
  margin: 0 auto;
  padding: 0 1rem 2rem 1rem;
}

.dashboard-nav {
  display: flex;
  justify-content: center;
  gap: 0.5rem;
  flex-wrap: wrap;
  margin: 0 0 1rem 0;
}

.dashboard-grid {
  display: grid;
  grid-template-columns: minmax(0, 0.8fr) minmax(400px, 1fr);
  gap: 1.5rem;
  align-items: start;
}

.dashboard-left {
  min-width: 0;
  overflow-x: hidden;
}

.dashboard-left .cell-output-display {
  max-height: 80vh;
  overflow-y: auto;
  overflow-x: hidden;
}

.dashboard-right {
  min-width: 320px;
  max-width: 100%;
  padding-top: 3.25rem;
  position: sticky;
  top: 1rem;
  overflow-x: hidden;
  box-sizing: border-box;
}

.dashboard-control {
  margin-bottom: 2rem;
}

.dashboard-control .observablehq,
.dashboard-control .cell-output,
.dashboard-player .observablehq,
.dashboard-player .cell-output {
  width: 100%;
  margin: 0 !important;
  padding: 0 !important;
}

.dashboard-player .observablehq > * {
  margin: 0 !important;
}

.dashboard-control label,
.dashboard-control .form-label {
  display: block;
  margin-bottom: 0.65rem;
  font-weight: 600;
}

.dashboard-control select {
  width: 100%;
  max-width: 100%;
}

.custom-iframe {
  width: 100%;
  max-width: 100%;
  min-height: 500px;
  display: block;
  border: 0;
  border-radius: 12px;
}

@media (max-width: 991px) {
  .dashboard-grid {
    grid-template-columns: 1fr;
    gap: 1.5rem;
  }

  .dashboard-right {
    padding-top: 0;
    position: static;
  }
}
</style>

<div class="dashboard-shell">
  <div class="dashboard-nav">
    <a class="btn btn-outline-light btn-sm rounded-pill px-3" href="about.html">ℹ️ About Me</a>
    <a class="btn btn-success btn-sm rounded-pill px-3" href="dashboard.html">📊 Hot 100 Chart</a>
    <a class="btn btn-outline-light btn-sm rounded-pill px-3" href="mood-of-the-week.html">🎧 Mood Snapshot</a>
  </div>
</div>
```

::: {.dashboard-shell}

```{=html}
<div class="dashboard-grid">
  <div class="dashboard-left">
```

## Billboard Hot 100 Chart

```{r}
#| echo: false

generate_svg_circle <- function(popularity_value) {
  popularity_value <- as.numeric(popularity_value)

  radius <- 10 + 15 * (popularity_value / 100)
  circle_colour_picker <- colorRampPalette(c("#B91d1d", "#ED8E11", "#EDDE11", "#1DB954"))
  color <- circle_colour_picker(101)[popularity_value + 1]

  sprintf(
    '<svg height="%1$s" width="%1$s"><circle cx="%2$s" cy="%2$s" r="%2$s" stroke="none" stroke-width="0" fill="%3$s" /><text class="circle-text" x="%2$s" y="%2$s" font-size="%4$s" fill="white" text-anchor="middle" dy=".3em">%5$s</text></svg>',
    2 * radius, radius, color, radius * 0.6, popularity_value
  )
}

hot100_df |>
  select(album_art, song_name, artist_name, track_duration, popularity) |>
  gt(id = "custom") |>
  cols_label(
    album_art = md("**Song**"),
    song_name = "",
    artist_name = md("**Artist**"),
    track_duration = html('<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="15" height="15" viewBox="0,0,255.998,255.998"><g fill="#ffffff" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt" stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0" font-family="none" font-weight="none" font-size="none" text-anchor="none" style="mix-blend-mode: normal"><g transform="scale(10.66667,10.66667)"><path d="M12,2c-5.523,0 -10,4.477 -10,10c0,5.523 4.477,10 10,10c5.523,0 10,-4.477 10,-10c0,-5.523 -4.477,-10 -10,-10zM14.586,16l-3.293,-3.293c-0.188,-0.188 -0.293,-0.442 -0.293,-0.707v-5c0,-0.552 0.448,-1 1,-1v0c0.552,0 1,0.448 1,1v4.586l3,3c0.39,0.39 0.39,1.024 0,1.414v0c-0.39,0.39 -1.024,0.39 -1.414,0z"></path></g></g></svg>'),
    popularity = md("**Popularity**")
  ) |>
  cols_width(
    album_art      ~ px(55),
    song_name      ~ px(190),
    artist_name    ~ px(120),
    track_duration ~ px(55),
    popularity     ~ px(80)
  ) |>
  text_transform(
    locations = cells_body(columns = album_art),
    fn = function(x) { web_image(url = x, height = 50) }
  ) |>
  text_transform(
    fn = function(x) { generate_svg_circle(x) },
    locations = cells_body(columns = popularity)
  ) |>
  tab_style(
    style = cell_text(whitespace = "normal"),
    locations = cells_body(columns = c(song_name, artist_name))
  ) |>
  fmt(
    columns = track_duration,
    fns = function(x) {
      num_minutes <- floor(x / 60000)
      num_seconds <- ifelse(round((x %% 60000) / 1000) == 60, 59, round((x %% 60000) / 1000))
      sprintf("%d:%02d", num_minutes, num_seconds)
    }
  ) |>
  cols_align(
    align = "center",
    columns = popularity
  ) |>
  tab_footnote(
    footnote = "The popularity of a track is a value between 0 and 100, with 100 being the most popular. The popularity is calculated by algorithm and is based, in the most part, on the total number of plays the track has had and how recent those plays are. Generally speaking, songs that are being played a lot now will have a higher popularity than songs that were played a lot in the past.",
    locations = cells_column_labels(columns = popularity)
  ) |>
  tab_source_note(
    source_note = md("**Source:** [Billboard Hot 100 Playlist](https://open.spotify.com/playlist/6UeSakyzhiEt4NB3UAd6NQ), from the Spotify Web API")
  ) |>
  tab_options(
    table.font.color = "#FFFFFF",
    table.background.color = "#191414",
    table.font.size = px(12),
    table.border.top.color = "transparent",
    table.border.bottom.color = "transparent",
    table_body.hlines.color = "transparent",
    table_body.border.bottom.color = "transparent",
    column_labels.border.bottom.color = "transparent",
    column_labels.border.top.color = "transparent",
    footnotes.background.color = "#282828",
    source_notes.background.color = "#282828"
  ) |>
  tab_style_body(
    style = cell_borders(
      sides = c("top", "bottom"),
      weight = px(0)
    ),
    fn = function(x) { is.numeric(x) | is.character(x) }
  ) |>
  opt_css(
    css = "
    table tr:nth-child(odd) {
      background-color: #282828;
    }

    table tr:hover {
      background-color: #383838;
    }

    #custom .gt_table {
      table-layout: fixed;
      width: 100%;
    }
    "
  )
```

```{=html}
  </div>
  <aside class="dashboard-right">
    <div class="dashboard-control">
```

```{ojs}
//| echo: false

hot100 = await FileAttachment("data/hot100_ojs.json").json()

viewof songDropdown = Inputs.select(
  hot100,
  {
    label: "Choose a song",
    format: d => d.song_name,
    unique: true
  }
)
```

```{=html}
    </div>
    <div class="dashboard-player">
```

```{ojs}
//| echo: false

html`<iframe
  class="custom-iframe"
  src=${`https://open.spotify.com/embed/track/${songDropdown.song_id}?utm_source=generator&theme=0`}
  width="100%"
  height="500"
  frameborder="0"
  allowfullscreen=""
  allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"
  loading="lazy">
</iframe>`
```

```{=html}
    </div>
  </aside>
</div>
```

:::
